        ...
        with open('./mumax_scripts/' + filename, 'w') as f1:
            ...
            for i in range(1, x*y+1):
                if i not in empty_space:
                    lines += 'm.SetRegion(' + str(i) + ', ' \
                          + configs[x0[i-1]] + ')\n'
                    # If the cell is initialised empty, set it as an actual
                    # empty cell with no magnetic saturation (as in vacuum)
                    if x0[i-1] == 0:
...
def find_max_B(file_path: str):
    ovf_path = './mumax_scripts/' + file_path + '.out/' + \
               'strayfield_optimise.ovf'
...
def simulated_annealing(x0: [int], T: float, T_min: float, alpha: float,
                        x: int, y: int, empty_space: [int]):
    max_costs = []
    max_actions = []
    max_cost = 0.0
    ...
    while T > T_min:
        ...
        while(count < 200):
            ...
            while not os.path.exists('./mumax_scripts/' +
                                     filename.split('.')[0] + '.out'):
                time.sleep(5)
...
x0 = [0, 2, 4, 1, 4, 3, 0, 2, 4, 4, 3, 2, 0, 1, 3, 1, 4, 3, 0, 4, 3, 3, 4, 2, 0, 3, 2, 1, 3, 0, 0, 1, 4, 3, 4, 3, 0, 1, 2, 3, 1, 4, 0, 1, 2, 2, 1, 1, 0, 2, 3, 1, 1, 1, 0, 4, 1, 2, 4, 4, 0, 4, 3, 2, 2, 0, 0, 3, 2, 1, 4, 3, 0, 4, 3, 1, 4, 3, 0, 0, 1, 2, 3, 2, 0, 1, 1, 4, 1, 3, 0, 2, 3, 4, 4, 3, 0, 2, 4, 3, 1, 0, 0, 1, 3, 1, 4, 1, 0, 2, 4, 2, 2, 3, 0, 3, 1, 1, 1, 3, 0, 4, 1, 1, 4, 1, 0, 1, 1, 3, 3, 4, 0, 3, 3, 2, 4, 4, 0, 4, 3, 3, 3, 2]
# x0 = x0.split(' ')
